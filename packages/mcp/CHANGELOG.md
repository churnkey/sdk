...
- Switching a draft offer's type via `update_blueprint_offer` now reliably removes the previous type's config (e.g. DISCOUNT→PAUSE drops `discountConfig`). The working-copy steps are normalized to plain objects before mutation; previously the Mongoose subdocuments didn't deep-clone cleanly, so the `delete` of the old config didn't persist and stale config blocks accumulated on the offer.
- `update_blueprint_offer` enforces that discount `autoOptimize` (adaptive discounts) requires Churnkey Intelligence access — the API rejects it (403) for orgs without `hasIntelligenceAccess` (or grandfathering), mirroring the dashboard, which hides the toggle. The error advises upgrading to an Intelligence plan.
- `update_blueprint_offer` can now add an offer to a target (offer step, survey choice, or structured follow-up option) that has none — pass `offerType` and the offer is seeded with that type and its default config, mirroring the dashboard's add-offer affordance. Editing an offer-less target without `offerType` is still rejected.
...
- Segment filter operands are scoped to the attribute value type, mirroring the dashboard: STRING/BOOLEAN use `INCLUDES`/`NOT_INCLUDES`; NUMBER/DATE use `GTE`/`LTE`/`BETWEEN`/`NOT_BETWEEN`. `GT`/`LT` are no longer offered (the builder never exposed them, so they rendered as unrecognized operands); `update_segment_filter` rejects an operand the value type doesn't allow, and the `operand` enum drops `GT`/`LT`.
...
- Segment filter rules on built-in attributes no longer persist a `type` field (the API strips it). `type` is only kept for custom attributes; on a built-in it is redundant (the evaluator derives the type) and previously caused the dashboard builder to render a duplicate value input next to the attribute's own control.

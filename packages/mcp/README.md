...
| `get_blueprint` | Full cancel flow blueprint by ID, including the `steps` array (each step/offer/survey-choice carries its `guid` for use with `update_blueprint_step`). Offers arrive with their provider objects resolved: `discountConfig.coupon` (name, percent or amount off, currency, duration) and `planChangeConfig.plans` (customer-facing plan name, amount, currency, interval) alongside the raw ids. Large for translated blueprints. |

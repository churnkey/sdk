...
| `publish_blueprint` | Publish a draft blueprint as the live org/segment version. Segment-scoped publish requires at least one audience filter rule and first publish enables the segment automatically. Enabled offers are validated against the org payment provider, including Braintree's required `CHURNKEY_PAUSE` discount for pause offers. Requires `confirm: "publish"` and writes an audit log. |
...
Segment tools (`create_segment_flow`, `archive_segment`, `reorder_segments`, `set_segment_enabled`, `update_segment_filter`) act on **live** segment config directly (segments have no draft cycle), so each requires an explicit `confirm` literal and writes an audit log. Creating a segment flow does not publish its blueprint; publishing remains gated by `publish_blueprint`. A segment flow cannot be published or enabled until its segment has at least one audience filter rule. The first successful publish of a segment flow enables that segment automatically. To avoid dashboard divergence, enabled published segments cannot have audience filters edited directly; disable first or create a new segment flow. Unfinished A/B test segments cannot be archived, enabled/disabled, or have their audience edited, and reorder operations must keep A/B test pairs together.

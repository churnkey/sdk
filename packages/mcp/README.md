...
| `update_segment_filter` | Replace a segment's audience filter rules (whole-array replace). Enabled published segments and unfinished A/B test segments cannot be edited. Requires `confirm: "update_segment_filter"` and writes an audit log. |
...
Segment tools (`create_segment_flow`, `archive_segment`, `reorder_segments`, `set_segment_enabled`, `update_segment_filter`) act on **live** segment config directly (segments have no draft cycle), so each requires an explicit `confirm` literal and writes an audit log. Creating a segment flow does not publish its blueprint; publishing remains gated by `publish_blueprint`. A segment flow cannot be published or enabled until its segment has at least one audience filter rule. Segments are created enabled by default; publishing does not change a segment's enabled state (mirroring the dashboard) — use `set_segment_enabled` to toggle targeting. To avoid dashboard divergence, enabled published segments cannot have audience filters edited directly; disable first or create a new segment flow. Unfinished A/B test segments cannot be archived, enabled/disabled, or have their audience edited, and reorder operations must keep A/B test pairs together.

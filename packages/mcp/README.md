...
| `update_blueprint_offer` | Edit one offer's type and functional config (discount/pause/trial/redirect/plan-change/rebate) on a draft, by `stepGuid` (+ optional `choiceGuid`/`optionGuid`). Offer types are validated against the org payment provider. Writes an audit log. |

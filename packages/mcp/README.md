...
| `publish_blueprint` | Publish a draft blueprint as the live org/segment version. Segment-scoped publish requires at least one audience filter rule and first publish enables the segment automatically. Requires `confirm: "publish"` and writes an audit log. |
...
| `set_segment_enabled` | Enable/disable a segment (live targeting on/off). Enabling requires at least one audience filter rule. Requires `confirm: "set_segment_enabled"` and writes an audit log. |

  ...
  return [
    ...
    },
    {
      ...
      description: [
        ...
        '',
        "Offers come with their payment-provider objects resolved, so you can describe an offer without a second lookup. A DISCOUNT offer's `discountConfig.coupon` carries `name`, `percentOff` OR `amountOff` (smallest currency unit, e.g. cents) with its `currency`, plus `duration` (`once`/`forever`/`repeating`) and `durationInMonths`. A PLAN_CHANGE offer's `planChangeConfig.plans` describes each id in `options` as `{ id, name, amount, currency, interval, intervalCount, active }`, where `name` is the customer-facing plan name (the workspace's plan nickname, else the provider product name, else the price nickname).",
        '',
        'Resolution is best-effort: an id the provider no longer has (a deleted coupon or price) is absent from `coupon` / `plans` while the raw `couponId` / `options` stay. Treat a missing resolution as "not present at the provider", not as an empty discount. The blueprint itself is shared across live and test mode, but the coupons/plans are read from the provider in this session\'s mode, so a live coupon id can come back unresolved in a test-mode session.',
      ].join('\n'),
